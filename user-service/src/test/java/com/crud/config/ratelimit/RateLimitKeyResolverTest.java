...
class RateLimitKeyResolverTest {
    ...
    @Test
    void usesBasicAuthUsernameWhenPresent() {
        MockHttpServletRequest request = new MockHttpServletRequest();
        String credentials = Base64.getEncoder().encodeToString("alice:secret".getBytes(StandardCharsets.UTF_8));
        request.addHeader("Authorization", "Basic " + credentials);

        assertThat(rateLimitKeyResolver.resolve(request)).isEqualTo("user:alice");
    }

    @Test
    void ignoresMalformedBasicAuthAndFallsBackToIp() {
        MockHttpServletRequest request = new MockHttpServletRequest();
        request.addHeader("Authorization", "Basic !!!");
        request.setRemoteAddr("203.0.113.11");

        assertThat(rateLimitKeyResolver.resolve(request)).isEqualTo("ip:203.0.113.11");
    }
